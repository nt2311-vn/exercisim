...
def approx_average_is_average(hand: list[int]) -> bool:
    ...
    med = len(hand) // 2
    avg_first_last = hand[0] / 2 + hand[-1] / 2
    actual_avg = card_average(hand)

    if float(hand[med]) == actual_avg or avg_first_last == actual_avg:
        return True
...
def average_even_is_average_odd(hand: list[int]) -> bool:
    ...
    if len(hand) < 2:
        return False
    avg_even_index = card_average(hand[0::2])
    avg_odd_index = card_average(hand[1::2])

    return avg_even_index == avg_odd_index
