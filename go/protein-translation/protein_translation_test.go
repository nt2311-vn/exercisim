		...
		t.Run(tc.input, func(t *testing.T) {
			...
			case tc.expectedError != nil:
				if err != tc.expectedError {
					t.Fatalf(
						"FromCodon(%q) expected error: %v, got: %v",
						tc.input,
						tc.expectedError,
						err,
					)
			...
			case err != nil:
				t.Fatalf(
					"FromCodon(%q) returned unexpected error: %v, want: %q",
					tc.input,
					err,
					tc.expected,
				)
	...
	for _, tc := range rnaTestCases {
		t.Run(tc.input, func(t *testing.T) {
			...
			case tc.expectedError != nil:
				if err != tc.expectedError {
					t.Fatalf(
						"FromRNA(%q) expected error: %v, got: %v",
						tc.input,
						tc.expectedError,
						err,
					)
			...
			case err != nil:
				t.Fatalf(
					"FromRNA(%q) returned unexpected error: %v, want: %q",
					tc.input,
					err,
					tc.expected,
				)
